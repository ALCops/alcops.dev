...
| Variable | Must start with an uppercase letter or underscore followed by uppercase (single-letter names are exempt); must not contain %, &, !, ? | Local and global variables |
| Parameter | Must start with an uppercase letter or underscore followed by uppercase (single-letter names are exempt) | Procedure parameters |
