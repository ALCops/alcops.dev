...
Failed HTTP requests are retried by a later compilation. After connectivity recovers, the next compilation can load the source without restarting the language server. The original compilation keeps its defaults and diagnostic so its rules use consistent settings.
