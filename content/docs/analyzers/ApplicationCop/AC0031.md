...
The tables come from the most recent `SetTables` call on the same `DataTransfer` variable that reaches the `CopyFields`/`CopyRows` call **in the same procedure or trigger**, and only when both arguments are `Database::"Table Name"` literals. A later `SetTables` replaces the earlier one, so a variable that is reconfigured between copies charges each call only for the tables it actually transfers. When branches (`if`, `case`, a loop body) set different tables before the call, all of those tables count. The variable itself may be a local, a global (also when addressed as `this.MyDataTransfer`), or a parameter.

When the tables cannot be determined — no `SetTables` before the call in the same body (for example, configured in another procedure or by the caller), or an argument that is a variable, parameter, or expression instead of a `Database::` literal — no diagnostic is reported for that call.
